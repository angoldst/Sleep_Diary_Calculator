...
```{r, include = F}
diary <- read.csv(path(path_to_data, diary_name), header = TRUE, sep = ",", na.strings = c("", "NA","."," ", "   ", "99"), stringsAsFactors = F)
diary %<>% filter(record_id == subject)
...
#calculations
  ...
  mutate(TIB = difftime(out_bed, in_bed2, units=c('hours') ) %>% as.numeric ,
         ...
         Session = factor(redcap_event_name %>% str_remove('_arm_2'), ordered = T, levels = session_order),
         Session_numeric = Session %>% as.character %>% str_replace('etx', '5') %>% str_replace('st', '6') %>% str_remove('t') %>% as.numeric) %>%
...
melt(averages %>% select(-sd_date, -record_id, -SE, -TST, -TIB,-date_as_date), id.vars = c('Session_numeric', 'Session')) %>%
  ...
  geom_point( ) + 
  geom_path() + 
  scale_x_continuous(breaks = c(1:6), labels = c('T1', 'T2', 'T3', 'T4', 'ETX', '2MO'))
...
```{r}
print(system("git rev-parse HEAD", intern=TRUE))
