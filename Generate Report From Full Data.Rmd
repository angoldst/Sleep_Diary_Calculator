...
variable_key <-read.csv(path(here::here('keys', 'GW_Covid_sleep_diary_variables.csv')))
```
...
```{r}
#rename variables
to_rename = variable_key %>% filter(gw_variable_name %in% colnames(diary) & covid_variable_name!='') %>% pull(gw_variable_name) %>% as.character()

diary %<>% 
  rename_at(vars(as.character(to_rename)), ~variable_key %>% filter(gw_variable_name %in% to_rename) %>% pull(covid_variable_name) %>% as.character())
